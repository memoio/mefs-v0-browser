...
class AddrList{
  ...
  async show(addr){
    this.addr = addr
    let res = await back.get(`/addr/${addr}/10/1`)
    this.refresh(res.data)
    css.convertRemoveClassByGroup(dqa('.content>div'),5,'hide')
    app.page.history('/addrList')
  }
  getList(res){
    let html = ''
